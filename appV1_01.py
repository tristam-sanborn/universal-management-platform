...
def search(searchString):
    if searchString == "":
        ...
        userSearchConnection = mysql.connector.connect(
        host="localhost",
        user=session["username"],
        password=session["password"],
        database="ump_database"
    )
    userSearchCursor = userSearchConnection.cursor()
    userSearchCursor.execute("SELECT * FROM employees WHERE firstname = %s", str(searchString))
    firstNameSearch = userSearchConnection.fetchone()
    userSearchCursor.execute("SELECT * FROM employees WHERE lastname = %s", searchString)
    lastNameSearch = userSearchConnection.fetchone()
    userSearchCursor.execute("SELECT * FROM employees WHERE username = %s", searchString)
    usernameSearch = userSearchConnection.fetchone()
    userSearchCursor.execute("SELECT * FROM employees WHERE UID = %s", searchString)
    uidSearch = userSearchConnection.fetchone()
    userSearchCursor.execute("SELECT * FROM employees WHERE location = %s", searchString)
    print(firstNameSearch)
    userSearchCursor.close()
    userSearchConnection.close()
